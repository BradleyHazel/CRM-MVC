...
router.get("/", (req, res, next) => {
  ...
  Customer.find({ owner: req.user._id ? req.user._id : req.user.id })
    .then((customers) => {
      Invoice.find({ owner: req.user._id ? req.user._id : req.user.id }).then(
        (result) => {
          ...
          for (let c = 0; c < result.length; c++) {
            if (result[c].dueDate.slice(0, 4) == thisyear) {
              // this year
              if (
                parseInt(result[c].dueDate.slice(5, 7)) <= parseInt(thismonth)
              ) {
                // this month or less
              ...
              result[c].dueDate.slice(0, 4) ==
              (parseInt(thisyear) - 1).toString()
            ) {
              if (
                parseInt(result[c].dueDate.slice(5, 7)) > parseInt(thismonth)
              ) {
  ...
  Customer.find({ owner: req.body.owner, name: req.body.customer })
    .then((customer) => {
      ...
      generateLog(
        req.body.owner,
        req.body,
        "Invoice created from customer page"
      );
    })
    .catch(console.error);
});
